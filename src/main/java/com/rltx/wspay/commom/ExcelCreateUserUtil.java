...
public class ExcelCreateUserUtil {


    public static ResultInfo IDCardValidate(String IDStr) throws ParseException {
        ResultInfo resultInfo = new ResultInfo();
        String Ai = "";
        // 判断号码的长度 15位或18位
        if (IDStr.length() != 15 && IDStr.length() != 18) {
            resultInfo.setTipInfo("身份证号码长度应该为15位或18位。");
            resultInfo.setLegal(false);
            return resultInfo;
        }
        ...
        if (isNumeric(Ai) == false) {
            resultInfo.setTipInfo("身份证15位号码都应为数字 ; 18位号码除最后一位外，都应为数字。");
            resultInfo.setLegal(false);
            return resultInfo;
        }
        ...
        if (isDate(strYear + "-" + strMonth + "-" + strDay) == false) {
            resultInfo.setTipInfo("身份证出生日期无效。");
            resultInfo.setLegal(false);
            return resultInfo;
        }
        ...
        try {
            if ((gc.get(Calendar.YEAR) - Integer.parseInt(strYear)) > 150
                ...
                resultInfo.setTipInfo("身份证生日不在有效范围。");
                resultInfo.setLegal(false);
                return resultInfo;
            }
        ...
        if (Integer.parseInt(strMonth) > 12 || Integer.parseInt(strMonth) == 0) {
            resultInfo.setTipInfo("身份证月份无效");
            resultInfo.setLegal(false);
            return resultInfo;
        }
        if (Integer.parseInt(strDay) > 31 || Integer.parseInt(strDay) == 0) {
            resultInfo.setTipInfo("身份证日期无效");
            resultInfo.setLegal(false);
            return resultInfo;
        }
        ...
        if (areacode.get(Ai.substring(0, 2)) == null) {
            resultInfo.setTipInfo("身份证地区编码错误。");
            resultInfo.setLegal(false);
            return resultInfo;
        }

        if(isVarifyCode(Ai,IDStr)==false) {
            resultInfo.setTipInfo("身份证校验码无效，不是合法的身份证号码");
            resultInfo.setLegal(false);
            return resultInfo;
        }
